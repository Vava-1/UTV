...
class PendingOrder(Base):
    """Stores pending cart data server-side before Stripe checkout completes.
    
    SECURITY: This avoids putting the full cart into Stripe metadata (500 char limit).
    Only the UUID goes to Stripe; the webhook retrieves the full cart from here.
    """
    __tablename__ = "pending_orders"

    id = Column(String(36), primary_key=True)  # UUID
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    cart_data = Column(Text, nullable=False)  # JSON string
