...
export function AboutPage() {
  ...
  const features = [
    { icon: Music, title: 'Classical & Gospel Music', description: 'A curated collection bridging sacred classical traditions with contemporary gospel expressions.', color: 'text-yellow-500', bg: 'bg-yellow-400/10' },
    ...
    <div className="space-y-16">
      ...
      <section className="bg-blue-900 rounded-2xl border border-blue-700 p-8 md:p-12">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex items-center justify-center gap-2 text-yellow-500"><Heart size={16} /><span className="text-sm font-medium">United by One Voice</span></div>
        </div>
      </section>
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((f, i) => (
            <motion.div key={f.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              className="bg-blue-900 rounded-xl border border-blue-700 p-6 hover:border-blue-600 transition-all">
