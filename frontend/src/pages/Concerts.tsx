  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-[#ffffff] border border-[#d1d5db] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl"
      ...
      >
        ...
        <div className="relative p-6 border-b border-[#e5e7eb]">
          <button
            onClick={onClose}
            className="absolute top-4 right-4 w-8 h-8 bg-blue-800 rounded-full flex items-center justify-center text-blue-200 hover:text-yellow-400 hover:bg-blue-700 transition-colors"
          >
            <X size={16} />
          </button>
          ...
          {concert.event_date && (
            <p className="text-sm text-blue-200 mt-0.5 flex items-center gap-1">
        ...
        <div className="p-6 space-y-6">
          ...
          {concert.available_tickets !== null && concert.available_tickets <= 10 && concert.available_tickets > 0 && (
            <div className="flex items-center gap-2 text-yellow-500 bg-yellow-400/10 border border-yellow-400/20 rounded-lg px-4 py-3 text-sm">
              <AlertCircle size={14} />
          ...
          {/* Quantity Selector */}
          <div>
            <label className="block text-xs text-blue-200 mb-3 tracking-widest uppercase">Number of Tickets</label>
            <div className="flex items-center gap-4">
              <button
                onClick={() => setQuantity(q => Math.max(1, q - 1))}
                disabled={quantity <= 1}
                className="w-10 h-10 rounded-xl bg-blue-800 border border-blue-600 flex items-center justify-center text-white hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                <Minus size={16} />
              </button>
          ...
          {/* Price Breakdown */}
          <div className="bg-blue-900 rounded-xl border border-blue-700 p-4 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-blue-200">Price per ticket</span>
            ...
            <div className="border-t border-blue-700 pt-2 flex justify-between">
              <span className="font-semibold text-white">Total</span>
              <span className="text-xl font-bold text-yellow-500">${total}</span>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-4 bg-yellow-400 text-[#ffffff] font-bold tracking-widest uppercase rounded-xl hover:bg-yellow-500 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-60"
          >
            {loading ? (
          ...
          <p className="text-xs text-blue-400 text-center">Secure payment powered by Stripe. Your ticket will be emailed to you.</p>
        </div>
      </motion.div>
    </motion.div>
  );
          ...
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-blue-900 rounded-xl border border-blue-700 p-6 animate-pulse">
              <div className="flex gap-6">
                <div className="w-20 h-20 bg-blue-800 rounded-xl" />
                <div className="flex-1 space-y-3">
                  <div className="h-6 bg-blue-800 rounded w-1/3" />
                  <div className="h-4 bg-blue-800 rounded w-1/2" />
                  <div className="h-4 bg-blue-800 rounded w-1/4" />
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="space-y-6">
          ...
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full bg-blue-900 border border-blue-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-rose-500/50 transition-colors"
            />
          </div>
          ...
          <div className="space-y-4">
            {filtered.map((concert, i) => (
              <motion.div
                ...
                className="group bg-blue-900 rounded-2xl border border-blue-700 overflow-hidden hover:border-rose-500/30 transition-all duration-300"
              >
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="flex flex-col md:flex-row flex-1 p-6 gap-6">
                    ...
                    <div className="flex-1">
                      ...
                      {concert.description && (
                        <p className="text-sm text-blue-200 mb-3 line-clamp-2">{concert.description}</p>
                      )}
                      <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-sm text-blue-200">
                        {concert.venue && (
                        ...
                        {concert.available_tickets !== null && (
                          <span className={`flex items-center gap-1.5 ${concert.available_tickets <= 20 ? 'text-yellow-500' : ''}`}>
            ...
            >
              ...
              {searchQuery && (
                <button
                  ...
                  className="mt-6 px-6 py-3 bg-blue-800 text-white rounded-xl font-medium hover:bg-blue-700 transition-all flex items-center gap-2"
                >
