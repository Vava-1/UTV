...
const categories = [
  { icon: Music, label: 'Classical', desc: 'Timeless classical masterpieces', color: 'from-yellow-400/20 to-orange-500/20' },
  ...
  { icon: Drum, label: 'Rhythmic', desc: 'Percussion and rhythm focused', color: 'from-yellow-500/20 to-yellow-400/20' },
...
export function DiscoverPage() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {categories.map((category, index) => (
            ...
            >
              <Link to="/music">
                <div className={`group relative overflow-hidden rounded-xl bg-gradient-to-br ${category.color} border border-[#e5e7eb]/50 p-6 hover:border-yellow-400/30 transition-all duration-300 cursor-pointer`}>
                  <div className="relative z-10">
                    <category.icon size={32} className="text-yellow-400 mb-4 group-hover:scale-110 transition-transform duration-300" />
                    <h3 className="text-lg font-bold text-white mb-2">{category.label}</h3>
                    <p className="text-sm text-[#6b7280]">{category.desc}</p>
                  </div>
                  <div className="absolute inset-0 bg-[#ffffff]/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
      ...
      >
        <Link to="/library">
          <div className="group rounded-xl bg-[#f9fafb] border border-[#e5e7eb]/50 p-6 hover:border-yellow-400/30 transition-all duration-300">
            <Headphones size={28} className="text-yellow-400 mb-3" />
            <h3 className="text-lg font-bold text-white mb-2">Your Library</h3>
            <p className="text-sm text-[#6b7280]">Access your saved music and playlists</p>
          </div>
        </Link>
        <Link to="/concerts">
          <div className="group rounded-xl bg-[#f9fafb] border border-[#e5e7eb]/50 p-6 hover:border-yellow-400/30 transition-all duration-300">
            <Radio size={28} className="text-yellow-400 mb-3" />
            <h3 className="text-lg font-bold text-white mb-2">Live Concerts</h3>
            <p className="text-sm text-[#6b7280]">Upcoming performances and events</p>
          </div>
        </Link>
        <Link to="/books">
          <div className="group rounded-xl bg-[#f9fafb] border border-[#e5e7eb]/50 p-6 hover:border-yellow-400/30 transition-all duration-300">
            <Disc size={28} className="text-yellow-400 mb-3" />
            <h3 className="text-lg font-bold text-white mb-2">Books & Scores</h3>
            <p className="text-sm text-[#6b7280]">Musical literature and sheet music</p>
          </div>
        </Link>
      </motion.div>
    </PageWrapper>
