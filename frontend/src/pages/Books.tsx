...
import { BookOpen, Search, ShoppingCart, Download, User, Building, Compass } from 'lucide-react';
...
export function BooksPage() {
  ...
  useEffect(() => {
    api.get('/contents?content_type=book&page_size=50')
      .then(res => setBooks(res.data.items))
      .catch(() => showToast(t('books.errorLoading'), 'error'))
  ...
  const addToCart = async (id: number) => {
    if (!isAuthenticated) { 
      showToast(t('books.loginRequired'), 'warning'); 
      return; 
    }
    try { 
      await api.post('/orders/cart', { content_id: id, quantity: 1 }); 
      showToast(t('books.addedToCart'), 'success'); 
    } catch (e) { 
      showToast(t('books.errorAdding'), 'error'); 
    }
  };
    ...
    <PageWrapper title={t('nav.books')} subtitle={t('books.subtitle')} icon={<BookOpen size={32} />}>
      <div className="space-y-6">
                ...
                <div className="flex gap-2">
                  {book.price ? (
                    <button 
                      ...
                      className="flex-1 py-2 bg-emerald-500/20 text-emerald-400 rounded-lg text-sm font-medium hover:bg-emerald-500/30 transition-colors flex items-center justify-center gap-2"
                    >
        ...
        {filtered.length === 0 && (
          ...
          >
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-emerald-500/10 to-transparent flex items-center justify-center mb-6">
